...
class ConjuntoSuite extends FunSuite {
  ...
  test("Test de union (general)") {
    ...
    val conjuntoUnion:Conjunto = union(conjunto1, conjunto2)
  ...
  test("Test de filtrado") {
    ...
    //-1 y 2 si pertenecen porque están en el conjunto2
    assert(conjuntoFiltrado(-1))
    assert(conjuntoFiltrado(2))
  }
  ...
  test("Test para existe") {
    ...
    // Sólo existe un elemento que cumpla esta condición por lo que así sabremos si funciona
    assert(existe(conjunto, x => x ==3))
  }
  ...
  test("Test de map") {
    ...
    // No pertenece al conjunto porque el 10 no esta incluido
    assert(!resultado(35))
  }
